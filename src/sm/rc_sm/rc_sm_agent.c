 ...
 */
 ...
 // Global time scheduling parameters                                                                    (add)
 static time_scheduling_params_t current_time_sched = {.a = 4, .b = 2, .c = 4};
 static pthread_mutex_t time_sched_mutex = PTHREAD_MUTEX_INITIALIZER;

 
 // Function to get time scheduling parameters                                                           (add)
 void rc_get_time_scheduling_params(uint8_t* a, uint8_t* b, uint8_t* c)
 {
   assert(a != NULL);
   assert(b != NULL);
   assert(c != NULL);
  
   pthread_mutex_lock(&time_sched_mutex);
   *a = current_time_sched.a;
   *b = current_time_sched.b;
   *c = current_time_sched.c;
   pthread_mutex_unlock(&time_sched_mutex);
 }

 // Function to apply time scheduling control                                                           (add)
 static
 void apply_time_scheduling_ctrl(const rc_ctrl_msg_t* ctrl)
 {
   assert(ctrl != NULL);
   assert(ctrl->type == RC_CTRL_SM_V0_TIME_SCHEDULING);
  
   const time_scheduling_params_t* params = &ctrl->time_sched.params;
  
   // Validate parameters
   uint8_t total = params->a + params->b + params->c;
   if(total != 10) {
     printf("[RC SM AGENT] Invalid time scheduling parameters, sum = %d, should be 10\n", total);
     return;
  }
  
  // Update global parameters with mutex protection
  pthread_mutex_lock(&time_sched_mutex);
  current_time_sched.a = params->a;
  current_time_sched.b = params->b;
  current_time_sched.c = params->c;
  pthread_mutex_unlock(&time_sched_mutex);
  
  printf("[RC SM AGENT] Time scheduling updated: a=%d, b=%d, c=%d\n", 
 ...
 */
 ...
 // Modify the existing on_control_rc_sm_ag function to handle time scheduling:                                       （modify）
 static
 sm_ctrl_out_data_t on_control_rc_sm_ag(sm_agent_t const* sm_agent, sm_ctrl_req_data_t const* data)
 {
   ...
   // Process time scheduling control if applicable
   if (rc_ctrl.msg.type == RC_CTRL_SM_V0_TIME_SCHEDULING) {
     apply_time_scheduling_ctrl(&rc_ctrl.msg);
   }
